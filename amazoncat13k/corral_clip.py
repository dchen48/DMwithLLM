...
def language_model_outputs(text_titles, text_contents, sentence_model, t5_tokenizer, t5_model, entity_model, entity_embd, num_entities, cos, algo, device):
        ...
        indexed_titles = findElements(text_titles, algo_indices)
        indexed_contents = findElements(text_contents, algo_indices)
        
        start = time.time()
        
        predicted_entities = predict_labels(indexed_titles, indexed_contents, t5_model, t5_tokenizer, device)
...
class CorralFastIGWOrg(object):
    def __init__(self, *, eta, nalgos, min_prob, device):
        import numpy
        
        super(CorralFastIGWOrg, self).__init__()
        self.min_prob = min_prob
        self.eta = eta / nalgos
        self.invpalgo = torch.Tensor([ nalgos ] * nalgos).to(device)
    ...
    def sample(self, lm_predicted_labels, bandit_predicted_labels):
        ...
        invpalgo = torch.gather(input=self.invpalgo.unsqueeze(0).expand(N, -1),
                                ...
                                index=algo.unsqueeze(1))
        corral_action = algo * lm_predicted_labels + (1 - algo) * bandit_predicted_labels
        return corral_action, algo, invpalgo
...
def learnOnline(dataset, rank, initlr, tzero, epsilon, epsilontzero, batch_size, cuda, seed, llm_type, min_prob):
    ...
    for bno, (Xs, text_titles, text_contents, ys) in enumerate(generator):
        ...
        with torch.no_grad():
            ...
            lm_predicted_labels, llm_time, cos_time = language_model_outputs(text_titles, text_contents, sentence_model, t5_tokenizer, t5_model, entity_model, entity_embd, num_entities, cos, algo=algo, device = device)
            ...
            reward = get_reward(corral_sample, ys).unsqueeze(1).float().to(Zs.device)
