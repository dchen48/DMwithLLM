...
class CorralFastIGW(object):
    def __init__(self, *, eta, nalgos, min_prob, budget, device):
        import numpy
    ...
    def sample_algo(self, N, num_llms):
        ...
        if num_llms + torch.sum(algo)>self.budget:
            diff = num_llms + torch.sum(algo) - self.budget
            for i in range(N):
                if diff>0 and algo[i]==1:
                    diff = diff-1
                    algo[i]=0
...
def learnOnline(dataset, rank, initlr, tzero, epsilon, epsilontzero, batch_size, cuda, seed, llm_type, min_prob, budget):
    import time
    ...
    for bno, (Xs, ys, pre_texts, post_texts, text_entities) in enumerate(generator):
        ...
        if bandit_model is None:
            ...
            corral_sampler = CorralFastIGW(eta=0.05, nalgos=2, min_prob = min_prob, budget = budget, device=Xs.device)
            start = time.time()
    ...
    save_dir = save_dir = './results/corral_budget/llm_type_' + llm_type + '/min_prob_' + str(min_prob) + '/budget_' + str(budget) + '/seed_' + str(seed) +'/'
...
if __name__ == '__main__':
    ...
    learnOnline(mydata, initlr=1/3, tzero=1000, rank=50, epsilon=1, epsilontzero=10, batch_size=32, cuda=True, seed=args.seed, llm_type = args.llm_type, min_prob = args.min_prob, budget = args.budget)
