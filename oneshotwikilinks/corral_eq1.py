...
def load_model_and_tokenizer(model_name, device):
    """ Load the model and tokenizer, move the model to the specified device """
    tokenizer = T5Tokenizer.from_pretrained(model_name)
    model = T5ForConditionalGeneration.from_pretrained(model_name).to(device)
    return model, tokenizer

def generate_mask_fillings(sentences, model, tokenizer, device):
    """ Replace [MASK] in sentences with the most likely words, using additional contexts """
    # Prepare the input texts by incorporating context and reformulating the question
    input_texts = [
...
class CorralFastIGW(object):
    def __init__(self, *, eta, nalgos, min_prob, max_num_llms, device):
        import numpy
        
        super(CorralFastIGW, self).__init__()
        self.min_prob = min_prob
        self.max_num_llms = max_num_llms
        self.eta = eta / nalgos
    ...
    def sample_algo(self, N, num_llms):
        initial_probs = 1.0/self.invpalgo
                
        initial_probs[1] = initial_probs[1] * (max(1-num_llms/self.max_num_llms,0))
        initial_probs[0] = 1 - initial_probs[1]
...
def learnOnline(dataset, rank, initlr, tzero, epsilon, epsilontzero, batch_size, cuda, seed, llm_type, min_prob, max_num_llms):
    import time
    ...
    for bno, (Xs, ys, pre_texts, post_texts, text_entities) in enumerate(generator):
        ...
        if bandit_model is None:
            ...
            corral_sampler = CorralFastIGW(eta=0.05, nalgos=2, min_prob = min_prob, max_num_llms = max_num_llms, device=Xs.device)
            start = time.time()
        ...
        with torch.no_grad():
            ...
            algo, invpalgo = corral_sampler.sample_algo(N, num_llms)
    ...
    save_dir = save_dir = './results/reweighted/corral/llm_type_' + llm_type + '/min_prob_' + str(min_prob) + '/max_num_llms_' + str(max_num_llms) + '/seed_' + str(seed) +'/'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_num_llms', type=float, default=50000)
    parser.add_argument('--llm_type', type=str, default='base', help="Supports [base, small, large]")
    args = parser.parse_args()

    mydata = loadMyDataset(2000)

    learnOnline(mydata, initlr=1/3, tzero=1000, rank=50, epsilon=1, epsilontzero=10, batch_size=32, cuda=True, seed=args.seed, llm_type = args.llm_type, min_prob = args.min_prob, max_num_llms = args.max_num_llms)
