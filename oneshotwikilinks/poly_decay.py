...
class CorralPoly(object):
    def __init__(self, nalgos, init_prob, c_poly, device):
        import numpy
        
        super(CorralPoly, self).__init__()
    ...
    def sample_algo(self, N, t):
        ...
        initial_probs[1] = min(self.init_prob, self.c_poly/t)
...
def learnOnline(dataset, rank, initlr, tzero, epsilon, epsilontzero, batch_size, cuda, seed, llm_type, init_prob, c_poly):
    import time
    ...
    for bno, (Xs, ys, pre_texts, post_texts, text_entities) in enumerate(generator):
        ...
        if bandit_model is None:
            ...
            corral_sampler = CorralPoly(nalgos=2, init_prob = init_prob, c_poly = c_poly, device=Xs.device)
            start = time.time()
    ...
    save_dir = save_dir = './results_poly/llm_type_' + llm_type + '/init_prob_' + str(init_prob) + '/c_poly_' + str(c_poly) + '/seed_' + str(seed) +'/'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--c_poly', type=float, default=1000)
    parser.add_argument('--llm_type', type=str, default='base', help="Supports [base, small, large]")
    args = parser.parse_args()

    mydata = loadMyDataset(2000)

    learnOnline(mydata, initlr=1/3, tzero=1000, rank=50, epsilon=1, epsilontzero=10, batch_size=32, cuda=True, seed=args.seed, llm_type = args.llm_type, init_prob = args.init_prob, c_poly = args.c_poly)
