...
def language_model_outputs(pre_texts, post_texts, sentence_model, t5_tokenizer, t5_model, entity_model, entity_embd, num_entities, cos, algo, id, device):
        batch_size = len(pre_texts)
        algo_indices = algo == id
        ...
        return predicted_labels.detach(), llm_time
...
class CorralFastIGW(object):
    ...
    def sample(self, algo, invpalgo, lm_predicted_labels_small, lm_predicted_labels_large, bandit_predicted_labels):
        predicted_labels = torch.stack(([bandit_predicted_labels, lm_predicted_labels_small, lm_predicted_labels_large]))
        corral_action = torch.gather(predicted_labels, 0, algo.unsqueeze(0))
        corral_action = corral_action.squeeze(0)
    ...
    def sample_algo(self, N):
        ...
        ratio_small = (initial_probs[1])/(initial_probs[1]+initial_probs[2]+1e-10)
        
        initial_probs[1] = (1 - initial_probs[0]) * ratio_small
        initial_probs[2] = (1 - initial_probs[0]) * (1- ratio_small)
...
def learnOnline(dataset, rank, initlr, tzero, epsilon, epsilontzero, batch_size, cuda, seed, min_prob):
    import time
    ...
    avloss_bandit, sincelast_bandit, avreward_bandit, avrewardsincelast_bandit, avreward_bandit, rewardsincelast_bandit, avrewardsincelast_llm_small, avrewardsincelast_llm_large = [ EasyAcc() for _ in range(8) ]
    avloss_llm, sincelast_llm, avreward_llm, avrewardsincelast_llm, avreward_llm, rewardsincelast_llm = [ EasyAcc() for _ in range(6) ]
    avloss_corral, sincelast_corral, avreward_corral, avrewardsincelast_corral, avreward_corral, rewardsincelast_corral, algo_sincelast_corral, algo_corral_small, algo_corral_large = [ EasyAcc() for _ in range(9) ]
    ...
    algo_corral_small_list = []
    algo_corral_large_list = []
    ...
    t5_model_small, t5_tokenizer_small = load_model_and_tokenizer(model_name_small, device)
    
    model_name_large = "google/flan-t5-large"
    
    t5_model_large, t5_tokenizer_large = load_model_and_tokenizer(model_name_large, device)
    ...
    for bno, (Xs, ys, pre_texts, post_texts, text_entities) in enumerate(generator):
        ...
        if bandit_model is None:
            ...
            corral_sampler = CorralFastIGW(eta=0.05, nalgos=3, min_prob = min_prob, device=Xs.device)
            start = time.time()
        ...
        with torch.no_grad():
            ...
            num_llms_small += torch.sum((algo==1).float()).item()
            num_llms_large += torch.sum((algo==2).float()).item()
            
            lm_predicted_labels_small, llm_time_small = language_model_outputs(pre_texts, post_texts, sentence_model, t5_tokenizer_small, t5_model_small, entity_model, entity_embd, num_entities, cos, algo=algo, id = 1, device = device)
            
            lm_predicted_labels_large, llm_time_large = language_model_outputs(pre_texts, post_texts, sentence_model, t5_tokenizer_large, t5_model_large, entity_model, entity_embd, num_entities, cos, algo=algo, id = 2, device = device)
            
            
            
            llm_times_small += llm_time_small
            llm_times_large += llm_time_large
                        
            corral_sample = corral_sampler.sample(algo, invpalgo, lm_predicted_labels_small, lm_predicted_labels_large, bandit_sample)
        ...
        with torch.no_grad():
            ...
            algo_indices_small = (algo == 1).nonzero().view(-1)
            algo_indices_large = (algo == 2).nonzero().view(-1)
            algo_sincelast_corral += len(algo_indices_small)/len(algo) + len(algo_indices_large)/len(algo)
            algo_corral_small += len(algo_indices_small)/len(algo)
            algo_corral_large += len(algo_indices_large)/len(algo)
        ...
        if bno % 100 == 0:
            ...
            algo_corral_list.append(algo_sincelast_corral.mean())
            
            algo_corral_small_list.append(algo_corral_small.mean())
            algo_corral_large_list.append(algo_corral_large.mean())
            ...
            sincelast_corral, avrewardsincelast_corral, rewardsincelast_corral, algo_sincelast_corral, algo_corral_small, algo_corral_large = [ EasyAcc() for _ in range(6) ]
    ...
    results = {'steps':avreward_steps, 'avreward_bandit':avreward_bandit_list, 'avrewardsincelast_bandit':avrewardsincelast_bandit_list, 'avreward_corral':avreward_corral_list, 'avrewardsincelast_corral':avrewardsincelast_corral_list, 'algo_corral':algo_corral_list,  'algo_corral_small':algo_corral_small_list,'algo_corral_large':algo_corral_large_list, 'num_llms_small':num_llms_small, 'num_llms_large':num_llms_large}
    
    save_dir = save_dir = './results_more_bandits/corral/min_prob_' + str(min_prob) + '/seed_' + str(seed) +'/'
...
if __name__ == '__main__':
    ...
    learnOnline(mydata, initlr=1/3, tzero=1000, rank=50, epsilon=1, epsilontzero=10, batch_size=32, cuda=True, seed=args.seed, min_prob = args.min_prob)
