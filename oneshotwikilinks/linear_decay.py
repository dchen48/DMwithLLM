...
class CorralLinear(object):
    def __init__(self, nalgos, init_prob, device):
        import numpy
        
        super(CorralLinear, self).__init__()
    ...
    def sample_algo(self, N, ratio):

        initial_probs = 1.0/self.invpalgo
        initial_probs[1] = self.init_prob * (1-ratio)
...
def learnOnline(dataset, rank, initlr, tzero, epsilon, epsilontzero, batch_size, cuda, seed, llm_type, init_prob):
    import time
    ...
    for bno, (Xs, ys, pre_texts, post_texts, text_entities) in enumerate(generator):
        ...
        if bandit_model is None:
            ...
            corral_sampler = CorralLinear(nalgos=2, init_prob = init_prob, device=Xs.device)
            start = time.time()
        ...
        with torch.no_grad():
            ...
            algo, invpalgo = corral_sampler.sample_algo(N, bno/num_batches)
    ...
    save_dir = save_dir = './results_uniform/llm_type_' + llm_type + '/init_prob_' + str(init_prob) + '/seed_' + str(seed) +'/'
...
if __name__ == '__main__':
    ...
    learnOnline(mydata, initlr=1/3, tzero=1000, rank=50, epsilon=1, epsilontzero=10, batch_size=32, cuda=True, seed=args.seed, llm_type = args.llm_type, init_prob = args.init_prob)
