...
class DummySampler(object):
    def __init__(self, *, eta, nalgos, device):
        import numpy
...
def learnOnline(dataset, rank, initlr, tzero, epsilon, epsilontzero, batch_size, cuda, seed, llm_type):
    import time
    ...
    for bno, (Xs, ys, pre_texts, post_texts, text_entities) in enumerate(generator):
        ...
        if bandit_model is None:
            ...
            corral_sampler = DummySampler(eta=0.05, nalgos=2, device=Xs.device)
            start = time.time()
        ...
        with torch.no_grad():
            for pred, avreward, avrewardsincelast in zip([bandit_sample, lm_predicted_labels, corral_sample], [avreward_bandit, avreward_llm, avreward_corral], [avrewardsincelast_bandit, avrewardsincelast_llm, avrewardsincelast_corral]):
    ...
    results = {'steps':avreward_steps, 'avreward_bandit':avreward_bandit_list, 'avrewardsincelast_bandit':avrewardsincelast_bandit_list, 'avreward_llm':avreward_llm_list, 'avrewardsincelast_llm':avrewardsincelast_llm_list, 'avreward_corral':avreward_corral_list, 'avrewardsincelast_corral':avrewardsincelast_corral_list, 'algo_corral':algo_corral_list, 'num_llms':num_llms}
    
    save_dir = save_dir = './results/all_llm/llm_type_' + llm_type + '/seed_' + str(seed) +'/'
...
if __name__ == '__main__':
    ...
    learnOnline(mydata, initlr=1/3, tzero=1000, rank=50, epsilon=1, epsilontzero=10, batch_size=32, cuda=True, seed=args.seed, llm_type = args.llm_type)
