...
class CorralExp(object):
    def __init__(self, nalgos, init_prob, c_exp, beta, device):
        import numpy
        
        super(CorralExp, self).__init__()
    ...
    def sample_algo(self, N, t):
        #N = lm_predicted_labels.shape[0]

        initial_probs = 1.0/self.invpalgo
        initial_probs[1] = min(self.init_prob, self.c_exp*np.exp(-self.beta*t))
...
def learnOnline(dataset, rank, initlr, tzero, epsilon, epsilontzero, batch_size, cuda, seed, llm_type, init_prob, c_exp, beta):
    import time
    ...
    for bno, (Xs, ys, pre_texts, post_texts, text_entities) in enumerate(generator):
        ...
        if bandit_model is None:
            ...
            corral_sampler = CorralExp(nalgos=2, init_prob = init_prob, c_exp = c_exp, beta = beta, device=Xs.device)
            start = time.time()
        ...
        with torch.no_grad():
            ...
            algo, invpalgo = corral_sampler.sample_algo(N, bno+1)
    ...
    save_dir = save_dir = './results_exp/llm_type_' + llm_type + '/init_prob_' + str(init_prob) + '/c_exp_' + str(c_exp) + '/beta_' +str(beta) + '/seed_' + str(seed) +'/'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--init_prob', type=float, default=0.8)
    parser.add_argument('--c_exp', type=float, default=1000)
    ...
    learnOnline(mydata, initlr=1/3, tzero=1000, rank=50, epsilon=1, epsilontzero=10, batch_size=32, cuda=True, seed=args.seed, llm_type = args.llm_type, init_prob = args.init_prob, c_exp = args.c_exp, beta = args.beta)
