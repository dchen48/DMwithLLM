...
class CorralFastIGW(object):
    def __init__(self, *, eta, nalgos, gamma, device):
        import numpy
        
        super(CorralFastIGW, self).__init__()
        self.bernoulli_dist = torch.distributions.Bernoulli(probs=gamma)
        self.eta = eta / nalgos
    ...
    def sample_algo(self, N):
        is_uniform = self.bernoulli_dist.sample()
        is_uniform = bool(is_uniform.item())
        if is_uniform:
...
def learnOnline(dataset, rank, initlr, tzero, epsilon, epsilontzero, batch_size, cuda, seed, llm_type, gamma):
    import time
    ...
    for bno, (Xs, ys, pre_texts, post_texts, text_entities) in enumerate(generator):
        ...
        if bandit_model is None:
            ...
            corral_sampler = CorralFastIGW(eta=0.05, nalgos=2, gamma = gamma, device=Xs.device)
            start = time.time()
    ...
    save_dir = save_dir = './results/corral_smooth/llm_type_' + llm_type + '/gamma_' + str(gamma) + '/seed_' + str(seed) +'/'
...
if __name__ == '__main__':
    ...
    learnOnline(mydata, initlr=1/3, tzero=1000, rank=50, epsilon=1, epsilontzero=10, batch_size=32, cuda=True, seed=args.seed, llm_type = args.llm_type, gamma = args.gamma)
